...
export default function ShareModal({ open, onClose, shareUrl, chart, birth, highlight }: ShareModalProps) {
  ...
  const downloadImage = async () => {
    ...
    try {
      ...
      if (!dataURL) {
        alert('Tạo ảnh thất bại, vui lòng chụp màn hình hoặc thử lại');
        return;
      }
      downloadDataURL(dataURL, `TuViLaSo_${Date.now()}.png`);
    } finally {
        ...
        >
          ...
          >
            ...
            <div style={{ padding: '0 20px 20px', display: 'flex', flexDirection: 'column', gap: '10px' }}>
              ...
              >
                {copied ? '✓ Đã sao chép liên kết' : 'ὑ7 Sao Chép Liên Kết Lá Số'}
              </button>
