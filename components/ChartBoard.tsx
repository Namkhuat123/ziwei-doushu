...
import { BRANCHES, BRANCHES_VI, STEMS, PALACE_NAME_VI } from '@/lib/ziwei/constants';
    ...
    <div className="w-full select-none">
      ...
      >
        ...
        <h2 className="text-sm tracking-[0.25em] font-medium" style={{ color: 'var(--t-gold)' }}>
          {chart.birthInfo.name ? `${chart.birthInfo.name} · ` : ''}Lá Số Tử Vi
        </h2>
      </motion.div>
      ...
      >
        ...
        >
          ...
          <div className="text-center space-y-1">
            <div className="text-[9px] tracking-[0.3em] font-medium" style={{ color: 'var(--t-gold)' }}>Tử Vi Đẩu Số</div>
            <div className="text-[10px] space-y-0.5" style={{ color: 'var(--t-faint)' }}>
              <div>Mệnh Cung <span style={{ color: 'var(--t-gold)', opacity: 0.7 }}>{BRANCHES_VI[chart.mingGongBranch]}</span></div>
              <div>Thân Cung <span className="text-sky-500/70">{BRANCHES_VI[chart.shenGongBranch]}</span></div>
          ...
          {chart.currentDaXianIndex >= 0 && (() => {
            ...
            return (
              <div className="border border-purple-500/30 rounded-lg px-3 py-1.5 text-center"
                ...
                <div className="text-[9px] text-purple-500/60">{PALACE_NAME_VI[dx.palaceName] ?? dx.palaceName}</div>
              </div>
            );
          })()}
      ...
      >
        ...
        <span className="px-1.5 py-0.5 rounded-full" style={{ color: 'var(--t-faint)', border: '1px solid var(--t-border)' }}>
          Nhấn cung xem Tam Phương Tứ Chính
        </span>
      </motion.div>
    </div>
  );
