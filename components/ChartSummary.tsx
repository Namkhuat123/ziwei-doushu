...
import { BRANCHES_VI, STEMS_VI, STAR_NAME_VI, PALACE_NAME_VI } from '@/lib/ziwei/constants';
...
export default function ChartSummary({ chart }: ChartSummaryProps) {
  ...
  return (
    ...
    >
      ...
      >
      <div className="glass-heavy shadow-lg rounded-xl p-5">
        ...
        <div className="flex flex-wrap items-start gap-4">
          <div>
            <div className="text-[9px] mb-1.5" style={{ color: 'var(--t-faint)', opacity: 0.85 }}>Chính Tinh Mệnh Cung</div>
            <div className="flex items-center gap-1">
              {mingStars.length > 0 ? (
                mingStars.map(s => (
                  <span key={s} className="font-bold text-lg" style={{ color: 'var(--t-gold)' }}>{STAR_NAME_VI[s] ?? s}</span>
                ))
              ) : (
                <span className="text-sm" style={{ color: 'var(--t-faint)' }}>Vô Chính Diệu</span>
              )}
            </div>
            {nature && (
          ...
          {keywords.length > 0 && (
            <div>
              <div className="text-[9px] mb-1.5" style={{ color: 'var(--t-faint)', opacity: 0.85 }}>Tính Cách Đặc Trưng</div>
          ...
          <div className="ml-auto text-right">
            ...
            {currentDx && (
              <div className="text-[11px] text-purple-500 mt-0.5">
                {currentDx.startAge}~{currentDx.endAge} tuổi · {PALACE_NAME_VI[currentDx.palaceName] ?? currentDx.palaceName}
              </div>
            )}
          </div>
        </div>
        ...
        <div className="mt-4 pt-3 flex flex-wrap gap-x-4 gap-y-1 text-[10px]"
          ...
          <span>
            Âm lịch {chart.lunarInfo.lunarYear} năm{chart.lunarInfo.isLeapMonth ? ' nhuận' : ''}
            {chart.lunarInfo.lunarMonth} tháng {chart.lunarInfo.lunarDay} ngày
          </span>
          <span>Năm {STEMS_VI[chart.lunarInfo.yearStem]} {BRANCHES_VI[chart.lunarInfo.yearBranch]} · Giờ {BRANCHES_VI[chart.birthInfo.hour]}</span>
          <span>Mệnh Cung {BRANCHES_VI[chart.mingGongBranch]} · Thân Cung {BRANCHES_VI[chart.shenGongBranch]}</span>
        </div>
      </div>
      </motion.div>
      ...
      {siHuaSummary.length > 0 && (
        ...
        >
        <div className="glass shadow-md rounded-xl p-4">
          ...
          <div className="grid grid-cols-2 gap-2">
            {siHuaSummary.map(({ name, siHua, palaceName }) => {
              ...
              return (
                ...
                >
                  <span className="font-medium">{STAR_NAME_VI[name] ?? name}</span>
                  <div className="flex items-center gap-1.5 text-right">
                    <span style={{ opacity: 0.6 }} className="text-[9px]">{PALACE_NAME_VI[palaceName] ?? palaceName}</span>
                    <span className="font-bold">Hóa {SIHUA_VI[siHua] ?? siHua}</span>
                  </div>
                </div>
              );
            })}
      ...
      {patterns.length > 0 && (
        ...
        <div className="glass shadow-md rounded-xl p-4">
          <div className="text-[10px] tracking-widest mb-3 flex items-center gap-2" style={{ color: 'var(--t-faint)' }}>
            <span style={{ color: 'var(--t-gold)', opacity: 0.6 }}>◉</span>
            Phân Biệt Cách Cục
            <span className="text-[9px] ml-auto" style={{ color: 'var(--t-faint)', opacity: 0.75 }}>{patterns.length} cách cục</span>
          </div>
                  ...
                  <div className="flex items-center gap-2 mb-1.5">
                    ...
                    <div className="flex gap-1 ml-auto">
                      {p.palaces.slice(0, 2).map(pa => {
                        const viName = PALACE_NAME_VI[pa] ?? pa;
                        return (
                  ...
                  {p.conditions && (
                    <div className="mt-2 pl-3.5 space-y-0.5">
                      ...
                      {p.conditions.breaking && p.conditions.breaking.length > 0 && (
                        <div className="text-[9px] leading-relaxed" style={{ color: 'var(--t-text2)', opacity: 0.85 }}>
                          <span className="font-medium text-orange-500">Phá cách</span>
                          <span style={{ opacity: 0.6 }}> · </span>
          ...
          {chart.daXians.slice(0, 9).map((dx, i) => {
            ...
            return (
              ...
              >
                ...
                <div className="text-[9px] mt-0.5" style={{ opacity: 0.7 }}>{PALACE_NAME_VI[dx.palaceName] ?? dx.palaceName}</div>
              </div>
            );
          })}
        </div>
