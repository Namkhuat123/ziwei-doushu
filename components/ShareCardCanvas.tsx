...
import { STAR_NAME_VI, PALACE_NAME_VI } from '@/lib/ziwei/constants';
...
export default function ShareCardCanvas({ chart, birth, highlight }: ShareCardProps) {
  ...
  const mingMajorStars = mingPalace?.stars.filter(s => s.type === 'major').map(s => STAR_NAME_VI[s.name] ?? s.name) ?? [];
  const mingStarStr = mingMajorStars.length > 0 ? mingMajorStars.join(' · ') : 'Vô Chính Diệu';
    ...
    <div id="share-card" style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '14px', position: 'relative', zIndex: 1 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
          ...
          <div>
            <div style={{ fontSize: '15px', color: '#3d2f10', fontWeight: 600, letterSpacing: '0.12em', lineHeight: 1.2 }}>Lá Số Tử Vi</div>
            <div style={{ fontSize: '9px', color: '#a89b7c', letterSpacing: '0.18em', marginTop: '2px' }}>Chính tông Ni Hải Hạ · ZI WEI</div>
          </div>
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end' }}>
          <div style={{ fontSize: '10px', color: '#6b5d3f', letterSpacing: '0.05em' }}>
            {birth.year} · Ngày {birth.month} Tháng {birth.day} · {birth.hour.padStart(2,'0')}:{birth.minute.padStart(2,'0')}
      ...
      <div style={{ display: 'flex', gap: '16px', flex: 1, position: 'relative', zIndex: 1, minHeight: 0 }}>
        ...
        <div style={{
            ...
            return (
              <div key={i} style={{
                ...
                <div style={{
                  ...
                  fontSize: '8px', color: '#a89b7c',
                }}>
                  <div>{PALACE_NAME_VI[cell.palace?.name || ''] ?? cell.palace?.name}</div>
                  <div>{cell.isMing ? 'Mệnh' : cell.isShen ? 'Thân' : ''}</div>
                </div>
                {/* 主星 */}
                <div style={{
                  marginTop: '2px',
                  ...
                  {cell.majors.length > 0 ? (
                    cell.majors.map((s, j) => (
                      <div key={j} style={{
                        ...
                        {STAR_NAME_VI[s.name] ?? s.name}{s.siHua ? <span style={{ fontSize: '8px', color: '#c45a2d', marginLeft: '1px' }}>{s.siHua}</span> : ''}
                      </div>
                    ))
                  ) : (
                    <div style={{ fontSize: '9px', color: '#a89b7c', fontStyle: 'italic' }}>Trống</div>
                  )}
                </div>
              </div>
            );
          ...
          <div style={{
            ...
            <div style={{ fontSize: '10px', color: '#6b5d3f', marginTop: '6px' }}>Cung Mệnh · {mingBranchName}</div>
            <div style={{ fontSize: '10px', color: '#6b5d3f' }}>Cung Thân · {shenBranchName}</div>
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
          {/* 命宫主星 */}
          <div>
            <div style={{ fontSize: '10px', color: '#a89b7c', letterSpacing: '0.25em', marginBottom: '2px' }}>CUNG MỆNH · {mingBranchName}</div>
            <div style={{
              fontSize: '52px',
              fontWeight: 800,
              color: '#8b6a14',
          ...
          <div>
            {dx && (
              <div style={{
                ...
                <span style={{ fontWeight: 600 }}>{dx.startAge}–{dx.endAge} tuổi · {PALACE_NAME_VI[dx.palaceName] ?? dx.palaceName}</span>
              </div>
            )}
            <div style={{
              padding: '10px 12px',
