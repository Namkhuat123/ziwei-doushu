...
export function generateChart(birthInfo: BirthInfo): ZiweiChart {
  ...
  // ── 借对宫结构化字段（codex P0：避免文案层从自然语言反查借宫 info）──
  palaces.forEach(p => {
  ...
  const rawWuxingJuName = astrolabe.fiveElementsClass as string;
  const wuxingJu       = parseWuxingJu(rawWuxingJuName);
  const wuxingJuName   = JU_NAMES[wuxingJu] ?? rawWuxingJuName;
